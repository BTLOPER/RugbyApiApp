...
namespace RugbyApiApp
{
    internal class Program
    {
            ...
            while (true)
            {
                ...
                if (games.Any())
                {
                    ...
                    foreach (var game in games.TakeLast(10))
                    {
                        ...
                        Console.WriteLine($"  • {date} | {homeTeam} vs {awayTeam} @ {location}");
                    }
                }
        ...
        static async Task ViewAllGamesAsync()
        {
            ...
            while (true)
            {
                ...
                foreach (var game in pageItems)
                {
                    ...
                    var location = game.Venue ?? "TBD";
                    var date = game.Date?.ToString("yyyy-MM-dd") ?? "TBD";
                    Console.WriteLine($"  {date} | {homeTeam,-18} vs {awayTeam,-18} @ {location}");
                }
        ...
        static async Task AutoFetchAllIncompleteAsync()
        {
            ...
            Console.WriteLine("→ Fetching Games and Teams...");
        ...
        static async Task FetchAllGamesAndTeamsAsync()
        {
            ...
            if (!leagues.Any())
            {
                Console.WriteLine("⚠ No leagues in database. Skipping games and teams fetch.\n");
                return;
            }
            ...
            if (!seasons.Any())
            {
                Console.WriteLine("⚠ No seasons in database. Skipping games and teams fetch.\n");
                return;
            }
            ...
            if (!yearList.Any())
            {
                Console.WriteLine("⚠ No valid seasons found. Skipping games and teams fetch.\n");
                return;
            }

            int startYear = yearList.Last(); // oldest year
            int endYear = yearList.First(); // newest year

            Console.WriteLine($"Fetching games for {leagues.Count} leagues from {startYear} to {endYear}...\n");
            ...
            foreach (var league in leagues)
            {
                try
                {
                    ...
                    if (errorMessage != null)
                    {
                        Console.WriteLine($"⚠ {league.Name}: API Error - {errorMessage}");
                        continue;
                    }
                    ...
                    if (games != null && games.Count > 0)
                    {
                        ...
                        Console.WriteLine($"  ✓ {league.Name}: {leagueGames} games, {leagueTeams} teams");
                    ...
                    {
                        Console.WriteLine($"  ℹ {league.Name}: No games found");
                    }
                }
                catch (HttpRequestException ex)
                {
                    Console.WriteLine($"  ✗ {league.Name}: {ex.Message}");
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"  ✗ {league.Name}: {ex.Message}");
            ...
            Console.WriteLine($"✓ New teams created: {totalTeams}\n");
        }
